...
export default function Navbar({ currentPath, onNavigate }: NavbarProps) {
  ...
  const logoWidth = settings?.logoSettings?.logoWidth || 150;
  const logoHeight = settings?.logoSettings?.logoHeight || 40;
  ...
  return (
    <>
      <nav 
        id="main-navbar"
        className={`fixed top-0 left-0 right-0 z-40 h-20 transition-all duration-300 flex items-center px-4 md:px-12 ${
          isScrolled 
      ...
      >
        <div className="w-full max-w-7xl mx-auto flex items-center justify-between">
          ...
          {/* Logo with Crescent and Star logo */}
          <div 
          ...
          {/* User controls / authentication buttons */}
            ...
            {user ? (
              <div className="flex items-center gap-3">
                {isAdmin && (
                  <button
                    ...
                    className="flex items-center gap-2 bg-gold-primary/15 border border-gold-primary/40 text-gold-primary font-semibold hover:bg-gold-primary/35 px-4 py-2 rounded-xl transition-all font-sans cursor-pointer"
                  >
              ...
              <>
                <button
                  ...
                  className="border border-gold-primary text-gold-primary hover:bg-gold-primary hover:text-navy-deep font-sans font-semibold tracking-wide px-5 py-2 rounded-xl transition-all cursor-pointer"
                >
                  Daxil ol
                </button>
                <button
                  onClick={() => handleLinkClick('/register')}
                  className="bg-gold-primary hover:bg-gold-dark text-navy-deep hover:shadow-lg active:scale-97 font-sans font-bold tracking-wide px-5 py-2 rounded-xl transition-all cursor-pointer"
                >
                  Qeydiyyat
                </button>
              </>
          ...
          {/* Hamburger Drawer Action on Mobile */}
          <button
            onClick={() => setIsDrawerOpen(!isDrawerOpen)}
            className="lg:hidden p-2 text-slate-100 hover:text-white transition-colors cursor-pointer w-11 h-11 flex items-center justify-center"
